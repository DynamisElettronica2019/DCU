...
time_t time;
date_t date;
extern osSemaphoreId updateRtcValueSemaphoreHandle;
/* USER CODE END PV */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  /* USER CODE BEGIN Callback 1 */
	...
	if (htim->Instance == TIM5) {
		...
		/* Timestamp managing task */
		if(updateRtcValueSemaphoreHandle != NULL) {
			xSemaphoreGiveFromISR(updateRtcValueSemaphoreHandle, &xHigherPriorityTaskWoken);
	...
	if (htim->Instance == TIM6) {
		...
		getTimestampTimeFormRtc(&time);
		getTimestampDateFormRtc(&date);
		getTimestampTimeFormGps(&time);
		getTimestampDateFormGps(&date);
	}
