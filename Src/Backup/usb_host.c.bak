...
FRESULT closeResult;
FRESULT writeResult;
USBH_StatusTypeDef USB_InitError;
...
{
	uint8_t errorLetter = USB_INIT_ERROR;
	
	USB_InitError = USBH_Init(&hUsbHostHS, USBH_UserProcess, HOST_HS);			/* Init Host Library */
	USB_InitError = USBH_RegisterClass(&hUsbHostHS, USBH_MSC_CLASS); 				/* Add Supported Class */
	USB_InitError = USBH_Start(&hUsbHostHS);																/* Start Host Process */
	MX_DriverVbusHS(VBUS_ENABLE);																						/* Enable USB Vbus power */
	
	if(USB_InitError != USBH_OK) {
...
extern inline void USB_EventHandler(uint8_t USB_Event)
{
	...
	switch(USB_Event) {
		case DISCONNECTION_EVENT:
			...
			fatFsResult = FATFS_UnLinkDriver(USBHPath);
			DATA_ResetUsbReadyState();																		/* Update of the status packet */
			
			if(mountResult != FR_OK) {
				errorLetter = USB_UNMOUNT_ERROR;
				xQueueSend(ErrorQueueHandle, (void *)&errorLetter, (TickType_t)0); 		/* Add error to queue */
			}
			
			if(fatFsResult != 0) {
				errorLetter = USB_FATFS_UNLINK_ERROR;
				xQueueSend(ErrorQueueHandle, (void *)&errorLetter, (TickType_t)0); 		/* Add error to queue */
			}
			break;

		case CONNECTED_EVENT:
			fatFsResult = FATFS_LinkDriver(&USBH_Driver, USBHPath);
