...
void GPS_init(void){	
	uint8_t buffer1[] = "$PUBX,41,1,0007,0003,38400,0*20\r\n";
	uint8_t buffer2[] = {0xB5, 0x62, 0x06, 0x08, 0x06, 0x00,0x64, 0x00, 0x01, 0x00,0x01, 0x00,0x7A, 0x12};
	/*decidere quali messaggio escludere dall'invio*/
	HAL_UART_Transmit(&huart2, (uint8_t *)buffer1, SET_BAUDRATE_38400_LENGTH, 100);			/*setto il baudrate del gps a 38400*/
	GPS_USART2_UART_Init_38400();																																		/*reinizializzo la uart a 38400*/
	HAL_Delay(50);																																									/*necessario?*/
	HAL_UART_Transmit(&huart2,(uint8_t *)buffer2, 14, 100); /*SETTO IL FIX RATE A 10HZ*/
	
//	HAL_UART_Transmit(&huart2,(uint8_t *)"0xB5,0x62,0x06,0x08,0x00,0x00,0x0E,0x30", 8, 100);
//	HAL_UART_Receive_DMA(&huart2,received, 400);
}
