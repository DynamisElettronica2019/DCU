...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(autogearManager, autogearManagerTask, osPriorityAboveNormal, 0, 128);
  autogearManagerHandle = osThreadCreate(osThread(autogearManager), NULL);

  /* definition and creation of USB_OvercurrentManager */
  osThreadDef(USB_OvercurrentManager, USB_OvercurrentManagerTask, osPriorityAboveNormal, 0, 128);
  ...
  osThreadDef(sendDebugData, sendDebugDataTask, osPriorityNormal, 0, 512);
  sendDebugDataHandle = osThreadCreate(osThread(sendDebugData), NULL);

  /* definition and creation of CAN_SendManager */
  osThreadDef(CAN_SendManager, CAN_SendManagerTask, osPriorityAboveNormal, 0, 512);
